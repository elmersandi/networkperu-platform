...
import { ArrowRight, CheckCircle, Shield, Server, Network, MessageCircle, Zap, Building2, Users, Trophy, Clock, LucideIcon } from "lucide-react";
...
export default function Home() {
  ...
  // =====================================================================
  const [tituloHero, setTituloHero] = useState("Conectividad Crítica para la Amazonía.");
  const [subtituloHero, setSubtituloHero] = useState("Infraestructura de red diseñada para resistir y rendir en los entornos más exigentes de la región.");
  const [whatsappComercial, setWhatsappComercial] = useState("51993370797");

  // Reconstruimos los Slides dinámicamente inyectando la información del CMS al Slide principal
  const SLIDES: Slide[] = [
    { title: tituloHero, subtitle: subtituloHero, img: "/heronetworks.jpg" },
    { title: "Ciberseguridad Nivel Corporativo.", subtitle: "Protegemos el ADN digital de su empresa con auditorías y blindaje perimetral.", img: "/ciberseguridad.jpg" }
  ];

  // Efecto: Consultar la base de datos al cargar la página
  useEffect(() => {
    fetch('/api/configuracion')
      .then(res => res.json())
      .then(data => {
        if (data && data.id) {
          if (data.heroTitulo) setTituloHero(data.heroTitulo);
          if (data.heroSubtitulo) setSubtituloHero(data.heroSubtitulo);
          if (data.whatsapp) setWhatsappComercial(data.whatsapp);
        }
      })
      .catch(err => console.error("Error cargando CMS:", err));
  }, []);
  ...
  return (
    <div className="bg-slate-50 font-sans selection:bg-blue-600 selection:text-white overflow-x-hidden text-slate-900">
      ...
      <section className="relative h-[85vh] lg:h-screen min-h-[600px] w-full overflow-hidden bg-[#000000]">
        <AnimatePresence mode="wait">
          <motion.div key={currentSlide} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.2 }} className="absolute inset-0">
            <Image src={SLIDES[currentSlide].img} alt="Infraestructura Tecnológica" fill className="object-cover opacity-70 grayscale-[30%]" priority />
          </motion.div>
        </AnimatePresence>
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-6 h-full flex flex-col justify-center items-center md:items-start text-center md:text-left">
          <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="max-w-3xl flex flex-col items-center md:items-start">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              <Link href={`https://wa.me/${whatsappComercial}`} target="_blank" className="bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-600/30 text-white font-bold px-10 py-4 rounded-lg transition-all flex items-center justify-center gap-2 group text-sm md:text-base transform hover:-translate-y-1">
      ...
      <section className="py-24 bg-slate-50 px-6 border-b border-slate-100">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <ServiceCard index={1} icon={Network} title="Cableado Estructurado" desc="Certificación Fluke en Cat 6A y Fibra Óptica." points={["Certificación Oficial", "Peinado de Racks"]} />
            <ServiceCard index={2} icon={Shield} title="Seguridad de Datos" desc="Blindaje perimetral y auditorías." points={["Firewalls Fortinet", "VLANs Seguras"]} />
            <ServiceCard index={3} icon={Server} title="Soporte IT 24/7" desc="Mantenimiento preventivo y mesa de ayuda." points={["Atención en sitio", "Monitoreo Proactivo"]} />
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-white px-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="max-w-5xl mx-auto">
          <div className="bg-slate-50 rounded-3xl border border-slate-200 overflow-hidden flex flex-col lg:flex-row shadow-xl shadow-slate-200/50">
            <div className="lg:w-1/2 p-10 md:p-16 flex flex-col justify-center bg-slate-900 text-center lg:text-left items-center lg:items-start relative overflow-hidden">
              ...
              <div className="space-y-4 relative z-10 w-full flex flex-col items-center lg:items-start">
                <div className="flex items-center gap-3 text-white text-sm font-semibold"><CheckCircle size={20} className="text-green-400" /> Presupuesto formal en 24 horas.</div>
                <div className="flex items-center gap-3 text-white text-sm font-semibold"><CheckCircle size={20} className="text-green-400" /> Atención directa de especialistas.</div>
              </div>
            </div>
            <div className="lg:w-1/2 bg-white p-10 md:p-16">
              <form className="space-y-6">
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-2.5">Su Número de WhatsApp</label>
      ...
      {/* BLOQUE 6: BOTÓN FLOTANTE WHATSAPP DEL CMS */}
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 1.2, type: "spring" }} className="fixed bottom-6 right-6 z-50">
        <Link href={`https://wa.me/${whatsappComercial}`} target="_blank" className="bg-[#25D366] hover:bg-[#1EBE53] text-white p-4 rounded-full shadow-lg shadow-green-500/30 hover:scale-110 transition-transform flex items-center justify-center group relative">
